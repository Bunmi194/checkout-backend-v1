...
import { verifyAccountNumber, createTransferRecipient, initiateTransfer } from "../services/paystackWithdrawal";
...
export const verifyAccount = async (req: Request, res: Response) => {
    ...
    const userDetails = jwt.verify(token, JWT_SECRET!) as unknown as newJwtPayload;
    ...
    verifyAccountNumber(`${bankAccount}`, `${bank}`, async (err: any, response: any) => {
        // console.log("err: ", err);
...
export const generateOTPForWithdrawal = async (req: Request, res: Response) => {
    ...
    try {
     ...
     const { authorization } = req.headers;
     const { amount, bankAccount, idempotentKey, bank, nameOnAccount } = req.body;
    ...
    if(newGeneratedTransaction && newGeneratedTransaction.id){
     ...
     const generatedTransaction = await writeTransactionToDatabase(transaction) as unknown as TransactionData
...
export const createTransferRecipientMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    //validate inputs
    const errors = validateWithdrawalInputs.safeParse(req.body);
    if(errors.success === false) {
        return res.status(400).json({
    ...
    const { bankAccount, bank, nameOnAccount, amount, idempotentKey, otp } = req.body;
    const { authorization } = req.headers;
    //check idempotent key
    if(!authorization || !idempotentKey){
        return res.status(401).json({
    ...
    const completeUserDetails = await userExists(Number(userDetails.id)) as unknown as UserData;
...
export const initiateTransferMiddleware = async (req: Request, res: Response) => {
    try {
    ...
    const { bankAccount, bank, nameOnAccount, amount, idempotentKey } = req.body;
    ...
    if (!completeUserDetails || !completeUserDetails.id || !completeUserDetails.balance){
        return res.status(404).json({
            status: false,
            message: "User does not exist"
        });
    ...
    initiateTransfer(formData, async (err: any, result: any) => {
        console.log("err: ", err)
        // console.log("result: ", result)
        if(err) return res.status(500).json({ status: false, message: "Failed to complete withdrawal request"});
        const response = JSON.parse(result.body);
        if(response){
            //on success, debit user account
        const withdraw = withdrawalTransactionQuery(completeUserDetails.id!, Number(amount));
        if(!withdraw){
            return res.status(500).json({
                status: false,
                message: "Internal Server Error"

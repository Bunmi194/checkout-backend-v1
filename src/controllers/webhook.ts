        ...
        if(hash === req.headers['x-paystack-signature']){
            ...
            if(event.event === "charge.success"){
                ...
                verifyPaymentPaystack(referenceId, async (error:any, body:any) => {
                    ...
                    if(!transactionDetails || !transactionDetails.id || !transactionDetails.userId) return;
                    if(transactionDetails && transactionDetails.status === "completed") return;
                    const user = await userExists(transactionDetails.userId!) as unknown as UserData;

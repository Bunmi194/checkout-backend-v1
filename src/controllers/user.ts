...
const FLWTOKEN = process.env.FLUTTERWAVE_API_SECRET_KEY_TEST || "FLWSECK_TEST-25f9d923e1d66d20670c2846f88e4cdb-X";
...
export const verifyEmail = async (req: Request, res: Response) => {
  ...
  //decode jwt token
  const userEmail = jwt.verify(token, JWT_SECRET!) as unknown as newJwtPayload;
  ...
  const updatedUser = await updateUserRecord(user.id!, updatedUserRec) as unknown as UserData;
